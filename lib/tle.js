...
function parseTleText(text, norad) {
  ...
  // Sanity: NORAD in line 1 should match (spaces/padding ok)
  const idField = l1.substring(2, 7).trim();
  const idNum = parseInt(idField.replace(/^[A-Za-z]/, (c) => {
    // Alpha-5: A=10 … not needed for current ham sats
    return c;
  }, 10);
  if (Number.isFinite(idNum) && String(norad) !== String(idNum)) {
    // still accept if field contains the digits of norad
    if (!idField.includes(String(norad)) && String(norad) !== idField) {
      // soft check only — some sources pad differently
    }
  }
  return { name, l1, l2 };
}
  ...
  for (const url of urls) {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (catField !== id && !catField.endsWith(id) && catField !== id.padStart(5, "0")) {
          continue;
        }
        const l2 = lines[i + 1];
        if (!l2 || !l2.startsWith("2 ")) continue;
        const nameLine = i > 0 && !lines[i - 1].startsWith("1 ") && !lines[i - 1].startsWith("2 ")
          ? lines[i - 1]
          : "NORAD " + id;
 ...
 * Order: Celestrak → AMSAT → SatNOGS → local disk cache → throw.
 */

...
/** SmartSDR / Kenwood-style CAT over TCP: FA; (+ optional MD;). */
const CAT_MD = {
  1: "LSB",
  2: "USB",
  3: "CW",
...
function probeCatTcp(host, port) {
  return new Promise((resolve) => {
    const socket = new net.Socket();
    let buf = Buffer.alloc(0);
    let done = false;
    ...
    socket.connect(port, host, async () => {
      try {
        buf = Buffer.alloc(0);
        socket.write(Buffer.from("FA;", "ascii"));
        await sleep(500);
        let text = buf.toString("ascii");
        let m = text.match(/FA(\d{8,11})/i);
        if (!m) {
          // one retry — some CAT stacks need a wake
          buf = Buffer.alloc(0);
          socket.write(Buffer.from("FA;", "ascii"));
          await sleep(500);
          text = buf.toString("ascii");
          m = text.match(/FA(\d{8,11})/i);
        }
        if (!m) {
          finish(
            fail("Connected but no FA reply (is SmartSDR CAT on this port?)", {
              endpoint: host + ":" + port,
              transport: "tcp",
              protocol: "cat",
              reply: text.trim().slice(0, 80) || "(empty)",
            }),
          );
          return;
        }
        ...
        try {
          buf = Buffer.alloc(0);
          socket.write(Buffer.from("MD;", "ascii"));
          await sleep(400);
          const mdText = buf.toString("ascii");
          const mm = mdText.match(/MD([0-9A-Ea-e])/i);
          if (mm) {
            const key = mm[1].toUpperCase();
            mode = CAT_MD[key] || CAT_MD[parseInt(key, 10)] || key;
          }
        } catch (_) {}
        const mhz = formatMHz(hz);
        finish(
...
async function testRadioSide(sideCfg) {
  ...
  // SmartSDR / Kenwood CAT: probe FA; (and MD;) so Test UL/DL returns freqMHz
  return probeCatTcp(cat.host, cat.port);
}

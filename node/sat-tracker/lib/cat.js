/**
 * IC-705 CI-V CAT driver (standalone — not yet wired into state/server).
 *
 * Protocol: Icom CI-V over USB CDC ACM
 *   Default address : 0xA4
 *   Baud            : 19200
...
function writeRaw(data) {
  return new Promise((resolve) => {
    ...
    port.write(data, (err) => {
      if (err) {
        console.warn("CAT write failed:", err.message);
        resolve(false);
        return;
      }
...
async function sendCiv(command) {
  if (!port || !connected) throw new Error("CAT not connected");
  if (busy) throw new Error("CAT busy");
  ...
  try {
    ...
    if (!ok) throw new Error("CAT write failed");

    // Collect reply (up to ~150 ms)
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      const timer = setTimeout(() => {
        ...
        console.warn("CAT open timeout", config.CAT_DEVICE);
        done(false);
      }, 3000);
      ...
      p.open((err) => {
        if (err) {
          clearTimeout(timer);
          console.warn("CAT open failed:", err.message);
          done(false);
          return;
        }
        clearTimeout(timer);
        ...
        p.on("error", (e) => {
          console.warn("CAT error:", e.message);
        });
      ...
      console.warn("CAT exception:", e.message);
      done(false);
    }
  });
}
...
async function setFrequency(freqHz) {
  ...
  console.log("CAT set", (freqHz / 1e6).toFixed(6), "MHz →", reply.toString("hex"));
  ...
  if (reply.includes(0xfa)) {
    console.warn("CAT NG (FA) on setFrequency");
    return false;
  }
...
async function getFrequency() {
  ...
  console.log("CAT get →", reply.toString("hex"));
  ...
  try {
    ...
    console.warn("CAT getFrequency parse failed:", e.message);
    return null;
  }
}

...
async function programSplitVfOs(dlHz, ulHz, mods, force) {
  // Absolute sequence with exclusive ownership of the CAT link (caller holds push lock).
  await setSplit(false);
  await sleep(50);

  // 1) DL on current (RX) VFO
  let f = await setFreqMode(dl, dlHz, mods.dl, force);
  if (!f) return false;
  console.log("FT817 RX VFO (DL)", (Math.round(dlHz) / 1e6).toFixed(6), "MHz");
  ...
  if (!f) return false;
  console.log("FT817 TX VFO (UL)", (Math.round(ulHz) / 1e6).toFixed(6), "MHz");

  // 3) Split + return to RX
  await setSplit(true);
  ...
  // Verify RX VFO is on DL
  const got = await readFreq(dl);
  if (got != null) {
    console.log(
      "FT817 RX verify",
      (got / 1e6).toFixed(6),
      "MHz",
      Math.abs(got - Math.round(dlHz)) < 2000 ? "OK" : "MISMATCH",
    );
  }
  return true;
}
...
async function runPush(ulHz, dlHz) {
  ...
  if (dualMode()) {
    ...
    if (useSplit) {
      ...
      if (force || syncNeeded || !splitOn) {
        ok =
          (await programSplitVfOs(dlHz, ulHz, mods, force || syncNeeded)) && ok;
        console.log(
          "FT817 FM split — UL locked on TX VFO, tracking DL on RX VFO",
        );
      } else {

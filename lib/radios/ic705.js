...
function clearSyncNeeded(reason) {
  ...
  syncNeeded = false;
  syncOkStreak = 0;
  console.log("IC705 sync clear:", reason || "ok");
}
...
function setCtcss(which) {
  ...
  lastCtcssApplied = null;
  console.log("IC705 CTCSS", ctcssMode, activeCtcssHz());
...
async function applyCtcssToRadio() {
  ...
  try {
    if (hz != null) {
      ...
      console.log("IC705 CTCSS", hz, "Hz ON on", link.name);
    } else {
      await sendCiv(link, Buffer.from([0x16, 0x42, 0x00]));
      console.log("IC705 CTCSS OFF on", link.name);
    }
    lastCtcssApplied = key;
  } catch (e) {
    console.warn("IC705 CTCSS:", e.message);
  }
}
...
async function ensureSplit() {
  if (dualMode()) {
    ...
    if (splitOn && dl.connected) {
      try {
        ...
        splitOn = false;
        console.log("IC705 SPLIT OFF (dual mode — Radio 1 is DL only)");
      } catch (_) {}
    }
    return false;
  }
  ...
  try {
    ...
    splitOn = true;
    console.log("IC705 SPLIT ON (TX = VFO B)");
    return true;
  } catch (e) {
    return false;
  }
...
function adjustFine(delta, side) {
  ...
  if (side === "dl") {
    dlFineOffset += delta;
    dl.lastHz = null;
    console.log("IC705 DL fine", delta, "→", dlFineOffset);
  } else {
    ulFineOffset += delta;
    ul.lastHz = null;
    console.log("IC705 UL fine", delta, "→", ulFineOffset);
  }
  broadcastStatus();
}
...
module.exports = {
  meta: {
    id: "ic705",
    label: "Icom IC-705 (CI-V)",
    match(cfg) {
      if (typeof cfg.useIc705Serial === "function") return cfg.useIc705Serial();

...
async function programSplitVfOs(dlHz, ulHz, mods, force) {
  ...
  // Mode is per-VFO on the 817 — always SET_MODE on each VFO.
  async function setOnCurrent(hz, modeCode, label) {
    dl.lastMode = null;
    const f = await setFreqMode(dl, hz, modeCode, true);
    if (f) {
      console.log(
        "FT817",
        label,
  ...
  // 1) DL + mode on current VFO (becomes RX)
  if (!(await setOnCurrent(dlHz, mods.dl, "RX/DL"))) return false;

  // 2) Other VFO = UL + mode
  await toggleVfo(dl);
  await sleep(80);
  if (!(await setOnCurrent(ulHz, mods.ul, "TX/UL"))) return false;
  ul.lastHz = Math.round(ulHz);

  // 3) SPLIT ON on TX VFO, then toggle back to RX (DL)
  await setSplit(true);
  console.log("FT817 SPLIT ON");
  await sleep(80);
  await toggleVfo(dl);
  ...
  if (got != null) {
    const ok = Math.abs(got - targetDl) < 5000;
    console.log(
      "FT817 RX verify",
      (got / 1e6).toFixed(6),
      "MHz",

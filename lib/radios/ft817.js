  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      p.open((err) => {
        ...
        // Brief settle then probe — if read fails, CAT is not talking to the radio
        setTimeout(async () => {
          try {
            ...
            if (probe != null) {
              ...
              console.warn(
                "FT817",
                link.name,
                "CAT NO RESPONSE to read-freq (check baud/menu CAT RATE, cable/ACC, try 9600)",
              );
            }
          } catch (e) {
...
async function setFreqMode(link, hz, modeCode, force) {
  ...
  const frame = Buffer.concat([bcd, Buffer.from([CMD.SET_FREQ])]);
  if (force || syncNeeded) {
    console.log(
      "FT817 SET_FREQ",
      (target / 1e6).toFixed(6),
  ...
  // Verify — FT-817 read returns current VFO
  if (force || syncNeeded) {
    const got = await readFreq(link);
    if (got == null) {
      console.warn("FT817 SET_FREQ no read-back — radio not responding on CAT");
    } else {
      console.log(
        "FT817 SET_FREQ read-back",
        (got / 1e6).toFixed(6),
        "MHz",
        Math.abs(got - target) < 500
          ? "OK"
          : "MISMATCH (wanted " + (target / 1e6).toFixed(6) + ")",
      );
    }
  }
  link.lastHz = target;
...
async function pushFrequencies(ulHz, dlHz) {
  ...
  if (dualMode()) {
    ...
    if (hasUl && !hasDl) {
      ...
      // Full relative sequence every force/sync; on normal ticks only
      // update RX VFO (DL) for Doppler if we believe we are on RX.
      if (force || syncNeeded || !splitOn) {
        ok = (await programSplitVfOs(dlHz, ulHz, mods, true)) && ok;
      } else {
        // Tracking: update RX (DL) on current VFO, then TX (UL) on other

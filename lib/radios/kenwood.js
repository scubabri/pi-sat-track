/**
 * Kenwood CAT serial driver (TS-2000).
 *
 * Protocol: ASCII commands terminated with ';'.
 *   FAxxxxxxxxxxx; / FBxxxxxxxxxxx;  — VFO A / B frequency (11-digit Hz)
 *   MD1…MD5;                         — mode (1 LSB, 2 USB, 3 CW, 4 FM, 5 AM)
 *   FT0; / FT1;                      — TX on VFO A / B (split)
 *   FR0; / FR1;                      — RX on VFO A / B
 *   TN00…TN39;                       — CTCSS tone code
 *   TO0; / TO1;                      — tone encoder off / on
 *
 * SPLIT (single radio, CAT2 blank):
 *   VFO A = DL, VFO B = UL, FT1 when TX_SPLIT.
 *   Satellite mode on the radio is left OFF — we use normal split like
 *   most CAT sat setups.
 *
 * DUAL (CAT2_DEVICE set):
 *   Radio 1 = DL only, Radio 2 = UL only.
 */
...
function modesForCatalogMode(modeStr) {
  ...
  // SSB sat convention: UL LSB, DL USB
...
function setRadio(on) {
  console.log("Kenwood setRadio(" + on + ")");
  if (on) {
    radioOn = true;
...
function setCtcss(which) {
  ...
  lastCtcssApplied = null;
  console.log("Kenwood CTCSS", ctcssMode, activeCtcssHz());
...
async function applyCtcssToRadio() {
  ...
  try {
    if (hz != null) {
      ...
      console.log("Kenwood CTCSS", hz, "Hz ON on", link.name);
    } else {
      await sendCmd(link, "TO0", 60);
      console.log("Kenwood CTCSS OFF on", link.name);
    }
    lastCtcssApplied = key;
  } catch (e) {
    console.warn("Kenwood CTCSS:", e.message);
  }
}
...
async function ensureSplit() {
  if (dualMode()) {
    if (splitOn && dl.connected) {
      ...
      splitOn = false;
      console.log("Kenwood SPLIT OFF (dual — Radio 1 DL only)");
    }
    return false;
  }
  if (!txSplitEnabled()) {
  ...
  splitOn = true;
  console.log("Kenwood SPLIT ON (TX = VFO B)");
  return true;
}
...
module.exports = {
  meta: {
    id: "kenwood",
    label: "Kenwood CAT (TS-2000)",
    match(cfg) {
      if (typeof cfg.useKenwoodSerial === "function")
        return cfg.useKenwoodSerial();

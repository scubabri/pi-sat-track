...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // Exchange only when we *know* MAIN is on the wrong band (e.g. 70cm
    // while UL is 2m). bandGroup(null) is "other" — never treat that as
    // inverted or we exchange every tick and undo a good layout.
    if (ulTarget != null && dlTarget != null) {
      ...
      if (
        ...
        console.log(
          "IC9700 MAIN↔SUB exchange (MAIN on",
          haveMain,
          "need",
          wantMain + ")",
        );

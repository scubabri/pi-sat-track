...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    // ── Steady Doppler: fire-and-forget, alternate sides ──
    // LOCK on  → drive Main (DL) + Sub (UL)
    // LOCK off → drive Sub (UL) only; leave Main alone so the operator
    //            can tune the DL passband (Hamlib cannot read dial in SAT
    //            mode — 0x25 NAK — so we must not overwrite Main).
    dl.dualPhase = (dl.dualPhase || 0) + 1;
    const driveMain = locked && needDl;
    const driveSub = needUl; // UL always tracks Doppler (fixed uplink / TX)

    if (driveMain && driveSub) {
      // Alternate to keep CI-V load light
      if (dl.dualPhase % 2 === 1) {
        writeCmd(dl, "V Main");
        writeCmd(dl, "F " + dlT);
        dl.lastFreqHz = dlT;
    ...
    // !locked && !needUl: nothing to write (operator owns Main)
    return;
  }

...
let nextLosAz = null;
let parkHold = false;
/** Below min EL: freeze last pointing (when HOLD_AFTER_PASS). */
...
function updateTracking(look, aosAz, losAz) {
  ...
  if (look.el >= minEl) {
    ...
    // Below min EL — end of useful tracking for this pass.
    modeLocked = false;
    flipped = false;

    const holdInPlace =
      config.ROTOR_HOLD_AFTER_PASS === true ||
      config.ROTOR_HOLD_AFTER_PASS === undefined;

    if (holdInPlace) {
      // Leave desiredAz/desiredEl at last on-pass pointing. No LOS@0 slew.
      if (!postPassHeld) {
        postPassHeld = true;
        console.log(
          "GS-232 below horizon — hold last pointing",
          desiredAz != null ? Number(desiredAz).toFixed(1) : "-",
          "el",
          desiredEl != null ? Number(desiredEl).toFixed(1) : "-",
          "(ROTOR_HOLD_AFTER_PASS)",
        );
      }
      return;
    }
    ...
    postPassHeld = false;
    // Legacy: HOLD at LOS az, el=0 (do not preposition to AOS).
    const holdAz =

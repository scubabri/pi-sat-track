...
function updateTracking(look, aosAz, losAz, meta) {
      ...
      heldHoldEl =
        desiredEl != null && Number.isFinite(desiredEl)
          ...
          : flipped
            ? elMax()
            : 0;
    } else if (az != null && Number.isFinite(az)) {
      // GS-232 stores last read position as bare numbers (az/el)
      heldLosAz = normalizeAz(az);
      heldHoldEl =
        el != null && Number.isFinite(el)
          ? Math.max(0, Math.min(elMax(), Number(el)))
          : 0;
  ...
  // If we never had a target (antenna on cold, below horizon), leave desired*
  // unchanged so the poll loop does not invent motion.
  prepositioned = false;

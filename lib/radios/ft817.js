...
async function programSplitVfOs(dlHz, ulHz, mods, force) {
  ...
  console.log("FT817 split program start (slow, band prime, 2s settles)");
  ...
  // If display looks like UL already, toggle toward DL/A
  const targetUl = Math.round(ulHz);
  const targetDl = Math.round(dlHz);
  if (got != null && Math.abs(got - targetUl) < 50000) {
    console.log("FT817 looks like UL VFO — toggling toward A/DL");
    await toggleVfo(dl);
    await sleep(SETTLE_MS);
    got = await readFreq(dl);
    if (got != null)
      console.log("FT817 after toggle", (got / 1e6).toFixed(5), "MHz");
  }
  dl.vfo = "A";
  ...
  console.log("FT817 toggle → VFO A (RX)");
  await toggleVfo(dl);
  await sleep(SETTLE_MS);

  got = await readFreq(dl);
  if (got != null && Math.abs(got - targetDl) > 50000) {
    console.log(
      "FT817 still not on DL (",
      (got / 1e6).toFixed(5),
      "MHz) — toggle once more",
    );
    await toggleVfo(dl);
    await sleep(SETTLE_MS);
    got = await readFreq(dl);

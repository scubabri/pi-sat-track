...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // Absorb dial BEFORE commanding — otherwise Doppler overwrites MAIN
    // every ~500ms and poll never wins the race.
    if (!locked && !syncNeeded) {
      const cap = await captureDialOffsets();
      if (cap && cap.delta) {
        // This tick's ulHz/dlHz were computed before the new offsets.
        // Apply the same Δ so this push keeps the dialled frequency.
        if (cap.sat) {
          if (dlHz != null && Number.isFinite(dlHz)) dlHz += cap.delta;
        } else {
          if (ulHz != null && Number.isFinite(ulHz)) ulHz += cap.delta;
          if (dlHz != null && Number.isFinite(dlHz)) dlHz -= cap.delta;
        }
        broadcastStatus();
      }
...
/**
 * Read MAIN dial once before Doppler command (LOCK off).
 *
 * state.js inverting math:
 *   UL = f0*(2-df) - manualDlOffset + ulFineOffset
 *   DL = f0*df     + manualDlOffset + dlFineOffset
 * So for passband tune we only adjust manualDlOffset (never BOTH
 * ulFine and manualDl — that doubles UL and oscillates).
 *
 * SAT:  dial MAIN=DL → manualDlOffset += Δ
 * Linear: dial MAIN=UL → manualDlOffset -= Δ  (UL up ↔ DL down)
 */
async function captureDialOffsets() {
  ...
  try {
    if (satModeOn) {
      ...
      manualDlOffset = next;
      // Freeze lastHz at dial so we don't re-count before setLinkFrequency
      dl.lastHz = measured;
      console.log(
        "IC9700 VFO DL",
    ...
    // Linear: dial on MAIN = UL — only manualDlOffset (inverting link)

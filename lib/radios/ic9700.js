...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // Always re-apply modes for linear sats (radio may restore per-band last mode on band change)
    ul.lastMode = null;
    dl.lastMode = null;

    // Frequency first (drives band stack), then mode — MAIN = UL (LSB for RS-44)
    if (ulTarget != null) {
      const f = await setLinkFrequency(dl, ulTarget, true, "A");
      const m = await setLinkMode(dl, mods.ul, mods.ulName, true, "A");
      if (!m || !f) ok = false;
      console.log(
        "IC9700 MAIN (UL/TX)",
    ...
    if (dlTarget != null) {
      const f = await setLinkFrequency(dl, dlTarget, true, "B");
      const m = await setLinkMode(dl, mods.dl, mods.dlName, true, "B");
      if (!m || !f) ok = false;
      console.log(
        "IC9700 SUB (DL/RX)",

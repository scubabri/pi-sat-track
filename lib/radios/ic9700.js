...
async function pollVfo() {
  ...
  // Do not gate on lastPushAt — Doppler ~500ms was blocking poll entirely.
...
/**
 * Read MAIN dial and fold into offsets (LOCK off). Used by poll and
 * immediately before each Doppler push so the dial is never overwritten
 * before we can observe it.
 *
 * Returns { delta } on change (signed Hz on the tuned VFO), else null.
 *
 * SAT: MAIN=DL → manualDlOffset += Δ
 * Linear: MAIN=UL → ulFineOffset += Δ, manualDlOffset -= Δ (inverting)
 */
async function captureDialOffsets() {
  ...
  try {
    if (satModeOn) {
      ...
      if (Math.abs(delta) < VFO_THRESH_HZ || Math.abs(delta) > 50000)
        return null;
      ...
      return { delta, sat: true };
    }

    // Linear: dial on MAIN = UL
    if (ul.lastHz == null || !Number.isFinite(ul.lastHz)) return null;
    const measured = await getSelectedFrequency(dl, "A");
    if (measured == null || !isPlausibleRigHz(measured)) return null;
    if (bandGroup(measured) !== bandGroup(ul.lastHz)) return null;

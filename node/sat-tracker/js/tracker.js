...
function fmtFreq(hz) {
  ...
  return mhz + '.' + String(khz).padStart(3, '0') + '.' + String(hzz).padStart(3, '0');
}
...
function updateStationStatus() {
  ...
  if (elevEl) {
    elevEl.textContent = (cfg.elevation != null && cfg.elevation !== '')
      ? cfg.elevation + ' m'
      : '-';
  }
}
...
function aosSortKey(s) {
  ...
  if (s.key === currentSatKey && currentEl != null && currentEl >= 0) return -1e12;
...
function renderSatMenu(payload) {
  ...
  quick.slice(0, 40).forEach(s => {
    ...
    if (s.above) tip += ' — above horizon';
      ...
      tip += ' — AOS < 15 min';
      if (typeof s.secToAos === 'number') tip += ' (~' + Math.round(s.secToAos / 60) + 'm)';
    } else {
      tip += ' — heard (AMSAT)';
    }
    btn.title = tip;
  ...
  if (statusCat) {
    statusCat.textContent = (payload.satellites || []).length + ' - ' + (payload.catalogNote || '?');
  }
...
function updateModeSelect(modes, modeIndex) {
  ...
  const key = (modes || []).map(m => m.mode + '|' + m.uplink + '|' + m.downlink).join(';');
  if (key !== lastModesKey) {
    lastModesKey = key;
    sel.innerHTML = '';
    if (!modes || !modes.length) {
      ...
      modes.forEach((m, i) => {
        ...
        opt.textContent = freqs.length ? (label + '  ·  ' + freqs.join(' / ')) : label;
        sel.appendChild(opt);
      });
    }
  }
...
function applyFreqAndLook(msg) {
  ...
  let ulHz = (msg.ulHz != null && Number.isFinite(msg.ulHz)) ? msg.ulHz : parseToHz(msg.uplink);
  let dlHz = (msg.dlHz != null && Number.isFinite(msg.dlHz)) ? msg.dlHz : parseToHz(msg.downlink);
  ...
  if (ulDopEl) ulDopEl.textContent = ulDop != null ? 'Doppler ' + fmtDopplerMHz(ulDop) : '';
  if (dlDopEl) dlDopEl.textContent = dlDop != null ? 'Doppler ' + fmtDopplerMHz(dlDop) : '';
  ...
  const ulLabelEl = document.querySelector('#freq-ul') && document.querySelector('#freq-ul').previousElementSibling;
  const dlLabelEl = document.querySelector('#freq-dl') && document.querySelector('#freq-dl').previousElementSibling;
...
function connectTracker() {
  ...
  ws.onmessage = (ev) => {
    try {
      ...
      if (msg.look) {
        const sky = (msg.passes && msg.passes[0] && msg.passes[0].sky)
          ? msg.passes[0].sky
          : null;
...
function updateSatelliteStatus(state) {
  ...
  if (noradEl) noradEl.textContent = state.norad != null ? String(state.norad) : '-';
...
function updateSidebar(state) {
  ...
  if (state.passes && state.passes.length) {
    ...
    const needLock = !lastPass ||
      lastPass.sat !== state.sat ||
      now > new Date(lastPass.los).getTime() + 2000 ||
      Math.abs(new Date(p.aos).getTime() - new Date(lastPass.aos).getTime()) > 120000;
    ...
    if (maxEl) maxEl.textContent = (lastPass.maxEl || p.maxEl).toFixed(1) + ' deg';

...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  // LOCK on: Doppler Main+Sub. LOCK off: Doppler Sub only (Main = dial).
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    const needDl =
      dl.lastFreqHz == null ||
      Math.abs(dlT - dl.lastFreqHz) >= DUAL_FREQ_THRESH_HZ;
    const needUl =
      dl.lastSubHz == null ||
      Math.abs(ulT - dl.lastSubHz) >= DUAL_FREQ_THRESH_HZ;
    const needMode =
      dl.lastMode !== mods.dl || dl.lastSubMode !== mods.ul;
    const due = now - (dl.lastPushAt || 0) >= DUAL_PUSH_MIN_MS;

    // Initial handshake once per TCP connection
    ...
    if (!needDl && !needUl) return;
    if (!due && needDl && needUl) {
      // both sides only slightly off — wait for interval
      if (
        Math.abs(dlT - dl.lastFreqHz) < 150 &&
        Math.abs(ulT - dl.lastSubHz) < 150
      )
        return;
    }
    dl.lastPushAt = now;

    // Steady: fire-and-forget. LOCK off → Sub only (leave Main for dial).
    const driveMain = locked && needDl;
    const driveSub = needUl;

    if (driveMain && driveSub) {

/**
 * Yaesu CAT serial driver (FT-991 / FT-991A).
 *
 * Protocol: ASCII commands terminated with ';'.
 *   FAxxxxxxxxx; / FBxxxxxxxxx;  — set VFO A / B frequency (9-digit Hz)
 *   MD0n;                        — mode (1 LSB, 2 USB, 3 CW, 4 FM, 5 AM)
 *   FT0; / FT1;                  — TX on VFO A / B (split)
 *   FR0;                         — RX on VFO A
 *   CT0; / CT1;                  — CTCSS encoder off / on
 *   CN00; … CN50;                — CTCSS tone code
 *
 * SPLIT (single radio, CAT2 blank):
 *   VFO A = DL, VFO B = UL, FT1 (TX = VFO B) when TX_SPLIT.
 *
 * DUAL (CAT2_DEVICE set):
 *   Radio 1 = DL only, Radio 2 = UL only. No split on Radio 1.
 */
...
/** Standard Yaesu CN tone table (Hz → code 00–50). */
const TONE_TABLE = [
...
function deviceFor(link) {
  if (link.name === "ul" && dualMode()) {
    return {
      ...
      baud: config.CAT2_BAUD || config.CAT_BAUD || 38400,
    };
  }
  return {
    path: config.CAT_DEVICE || "/dev/ttyUSB0",
    baud: config.CAT_BAUD || 38400,
  };
}
...
function modesForCatalogMode(modeStr) {
  ...
  if (s.includes("FM")) return { ul: "4", dl: "4", ulName: "FM", dlName: "FM" };
  if (s.includes("CW")) return { ul: "3", dl: "3", ulName: "CW", dlName: "CW" };
  // SSB satellite: UL LSB, DL USB by convention
...
function writeRaw(link, data) {
  return new Promise((resolve) => {
    ...
    link.port.write(data, (err) => {
      if (err) {
        console.warn("Yaesu", link.name, "write failed:", err.message);
        resolve(false);
        return;
      }
...
function openLink(link) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      p.on("error", (e) =>
        console.warn("Yaesu", link.name, "error:", e.message),
      );
      ...
      const timer = setTimeout(() => {
        ...
        console.warn("Yaesu", link.name, "open timeout", path);
        done(false);
      }, 3000);
      ...
      p.open((err) => {
        if (err) {
          clearTimeout(timer);
          console.warn("Yaesu", link.name, "open failed:", err.message);
          done(false);
          return;
        }
        clearTimeout(timer);
        ...
        p.on("close", () => {
          console.log("Yaesu", link.name, "closed");
      ...
      link.connecting = false;
      console.warn("Yaesu", link.name, "exception:", e.message);
      done(false);
    }
  });
}
...
function setCtcss(which) {
  ...
  lastCtcssApplied = null;
  console.log("Yaesu CTCSS", ctcssMode, activeCtcssHz());
...
async function applyCtcssToRadio() {
  ...
  try {
    if (hz != null) {
      ...
      if (code != null) {
        await sendCmd(link, "CN" + String(code).padStart(2, "0"), 60);
      }
      await sendCmd(link, "CT1", 60);
      console.log("Yaesu CTCSS", hz, "Hz ON on", link.name);
    } else {
      await sendCmd(link, "CT0", 60);
      console.log("Yaesu CTCSS OFF on", link.name);
    }
    lastCtcssApplied = key;
  } catch (e) {
    console.warn("Yaesu CTCSS:", e.message);
  }
}
...
async function ensureSplit() {
  if (dualMode()) {
    if (splitOn && dl.connected) {
      ...
      splitOn = false;
      console.log("Yaesu SPLIT OFF (dual — Radio 1 DL only)");
    }
    return false;
  }
  if (!txSplitEnabled()) {
  ...
  await sendCmd(dl, "FT1", 60); // TX = VFO B
  await sendCmd(dl, "FR0", 60); // RX = VFO A
  splitOn = true;
  console.log("Yaesu SPLIT ON (TX = VFO B)");
  return true;
}
...
async function setFreq(link, hz, vfo) {
  ...
  const digits = String(target).padStart(9, "0").slice(-9);
...
async function setMode(link, mdCode, name, vfo) {
  ...
  // Select VFO then set mode (MD applies to current VFO on FT-991)
  if (vfo === "B") await sendCmd(link, "FR1", 40);
  else await sendCmd(link, "FR0", 40);
  const raw = await sendCmd(link, "MD0" + mdCode, 60);
  if (vfo === "B") await sendCmd(link, "FR0", 40); // back to A for RX
  if (raw == null) return false;
...
async function readVfoA(link) {
  ...
  if (!raw) return null;
  const m = String(raw).match(/FA(\d{8,9})/i);
  if (!m) return null;
...
module.exports = {
  meta: {
    id: "yaesu",
    label: "Yaesu CAT (FT-991)",
    match(cfg) {
      if (typeof cfg.useYaesuSerial === "function") return cfg.useYaesuSerial();

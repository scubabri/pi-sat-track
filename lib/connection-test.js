...
async function probeIcom(device, baud, civAddr) {
  ...
  try {
    ...
    // Find payload after FE FE ... 03
    let hz = null;
    for (let i = 0; i < buf.length - 5; i++) {
      if (buf[i] === 0x03 && i + 5 < buf.length) {
        hz = icomBcdFreqHz(buf.slice(i + 1, i + 6));
        if (hz && hz > 1e6) break;
      }
    }
    // Mode: FE FE addr E0 04 FD
    let mode = null;
    const mcmd = Buffer.from([0xfe, 0xfe, addr & 0xff, 0xe0, 0x04, 0xfd]);
    const mr = await serialTransact(port, mcmd, 400);
    if (!mr.error && mr.buf) {
      for (let i = 0; i < mr.buf.length - 1; i++) {
        if (mr.buf[i] === 0x04 && i + 1 < mr.buf.length) {
          mode = ICOM_MODE[mr.buf[i + 1]] || "0x" + mr.buf[i + 1].toString(16);
          break;
        }
      }
    }
...
async function testRadioSide(sideCfg) {
  ...
  if (type === "rigctl" || protocol === "rigctl") {
    ...
    if (r.ok) {
      // rigctl 'f' returns frequency in Hz
      let hz = null;
      if (r.detail && r.detail.reply) {
        const n = parseInt(String(r.detail.reply).trim(), 10);
        if (Number.isFinite(n) && n > 0) hz = n;
      }
      const mhz = formatMHz(hz);
      r.detail = Object.assign({}, r.detail || {}, {
        endpoint: ep.host + ":" + ep.port,
      ...
      r.message =
        "rigctl " + ep.host + ":" + ep.port + (mhz ? " — " + mhz + " MHz" : "");
    }
    return r;
  }
  if (protocol === "tci" || type === "aethersdr") {
    ...
    if (r.ok) {
      ...
      r.message = "TCI TCP " + ep.host + ":" + ep.port;
    }
    return r;
  }

 ...
 * N-stop: start AOS side; fold over-top when AZ would cross north.
 */
...
/** Prefer NORMAL (AOS side) unless already clearly over-top (EL > 100). */
...
/**
 * N-stop over-the-top:
 * 1. Start NORMAL on the AOS side (unless already over-top).
 * 2. If the shortest AZ path to the sat would cross north, switch to FLIPPED
 *    and stay there for the rest of the pass (EL goes through 90→180).
 */
function updateTracking(look, aosAz) {
  ...
  if (look.el >= config.ROTOR_MIN_EL) {
    if (!modeLocked) {
      ...
      const from =
        desiredAz != null && Number.isFinite(desiredAz)
          ? desiredAz
          : az.pos != null && Number.isFinite(az.pos)
            ? az.pos
            : null;
      if (from != null && shortestPathCrossesNorth(from, look.az)) {
        flipped = true;
        console.log(
          "Rotor mode → FLIPPED (N-stop over-top) az",
          look.az.toFixed(1),
          "el",
          look.el.toFixed(1),
        );

...
function updateTracking(look, aosAz, losAz, meta) {
  ...
  if (look.el >= minEl) {
    postPassHeld = false;
    prepositioned = false;
    // Capture / refresh LOS az at min EL while we still have this pass.
    if (nextLosAz != null && Number.isFinite(nextLosAz)) {
  ...
  // Freeze LOS min-EL az at the moment we leave the pass.
  if (!postPassHeld) {
    if (nextLosAz != null && Number.isFinite(nextLosAz)) {
      heldLosAz = normalizeAz(nextLosAz);
    } else if (look.az != null && Number.isFinite(look.az)) {
      heldLosAz = normalizeAz(look.az);
    } else if (desiredAz != null && Number.isFinite(desiredAz)) {
      heldLosAz = normalizeAz(desiredAz);
    }
    postPassHeld = true;
    console.log(
      "Rotor below min EL — hold LOS az",
      heldLosAz != null ? Number(heldLosAz).toFixed(1) : "-",
      "(az at min EL)",
    );
  }

  // 5 minutes before next AOS: preposition to next pass min-EL (AOS) az.
  // Checkbox "Preposition 5 min before next AOS" maps to ROTOR_HOLD_AFTER_PASS
  // (default true). When false, stay on held LOS az until Park / next pass.
  const allowPrep =
  ...
  if (
    ...
    const azN = normalizeAz(nextAosAzMeta);
    if (
      !prepositioned ||
      desiredAz == null ||
      ...
      console.log(
        "Rotor preposition for next AOS in",
  ...
  // Between passes: stay on held LOS min-EL az, EL at 0.

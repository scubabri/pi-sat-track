...
function useYaesuSerial() {
  if (RADIO_TRANSPORT !== "serial") return false;
  const make = String(SERIAL_MAKE || "").toLowerCase();
  const model = String(SERIAL_MODEL || "").toLowerCase();
  if (make && make !== "yaesu") return false;
  // Binary FT-817 family — not ASCII Yaesu driver
  if (
    model === "ft-817" ||
    model === "ft817" ||
    model === "ft-818" ||
  ...
  return !!(m && m.supported && m.driver === "yaesu");
}
...
function useKenwoodSerial() {
  ...
  return !!(m && m.supported && m.driver === "kenwood");
}

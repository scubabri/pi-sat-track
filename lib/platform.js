...
function defaultCatDevice() {
  if (isWindows()) {
    // Prefer a live COM port; never return a Linux /dev path
    try {
      const ports = listSerialDevicesWindows();
      if (ports && ports.length && /^COM\d+$/i.test(ports[0])) return ports[0];
    } catch (_) {}
    return "";
  }
  if (isDarwin()) return "/dev/cu.usbmodem0001";
  // Pi and generic Linux: ACM first (CDC), else USB-serial
  return "/dev/ttyACM0";
}

...
async function setVfo(link, vfo) {
  ...
  link.currentVfo = vfo;
  // Mode is per-VFO on dual rigs — force re-apply after switch
  if (vfo === "Sub" || vfo === "VFOB") link.lastSubMode = null;
  else link.lastMode = null;
  return true;
}
...
async function pushFrequencies(ulHz, dlHz) {
  ...
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    await setFreqSide(dl, dlHz, "main");

    // SUB = uplink (TX)
    await setVfo(dl, "Sub");
    await setMode(dl, mods.ul, "sub");
    await setFreqSide(dl, ulHz, "sub");

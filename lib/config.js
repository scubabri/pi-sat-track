...
/** Global minimum elevation (°) for tracking + pass AOS/LOS. Profile-overridable. */
let ROTOR_MIN_EL = parseFloat(process.env.ROTOR_MIN_EL || "0");
if (!Number.isFinite(ROTOR_MIN_EL)) ROTOR_MIN_EL = 0;
ROTOR_MIN_EL = Math.max(0, Math.min(90, ROTOR_MIN_EL));
...
function applyEndpoints(ep) {
  ...
  if (ep.rotorMinEl != null && Number.isFinite(Number(ep.rotorMinEl))) {
    const m = Math.max(0, Math.min(90, parseFloat(ep.rotorMinEl)));
    if (Number.isFinite(m) && m !== ROTOR_MIN_EL) {
      ROTOR_MIN_EL = m;
      rotorChanged = true;
    }
  }

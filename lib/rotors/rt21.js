 ...
 *      cross north, stay NORMAL even if the full AOS→LOS pass did —
 *      the sat is already past the N crossing.
 *   2. If remaining path crosses north → FLIPPED.
...
function chooseInitialMode(satAz, satEl) {
  ...
  if (nextLosAz != null && Number.isFinite(nextLosAz)) {
    ...
    // Remaining path is clear of N-stop → NORMAL (past the crossing)
 ...
 *   acquires past the north crossing stay NORMAL.

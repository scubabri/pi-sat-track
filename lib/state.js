...
async function loadSatellite(key) {
  ...
  // Clear any fine/manual offsets from the previous sat (e.g. RS-44 dial absorb)
  // BEFORE the TLE await so a tick cannot re-absorb while lock is still the
  // previous sat's unlocked state.
  ...
  // Reset again after TLE load in case a concurrent tick re-absorbed during await
  radios.resetAllOffsets();
  const active = getActiveMode(info);
  applyLockDefaultForMode(active && active.mode);
  applyCtcssDefaultForMode(info, active);

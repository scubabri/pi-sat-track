...
function updateTracking(look, aosAz, losAz, meta) {
  ...
  if (look.el >= minEl) {
    postPassHeld = false;
    prepositioned = false;
    // While tracking, keep refreshing the planned LOS az for this pass only.
    if (nextLosAz != null && Number.isFinite(nextLosAz)) {
  ...
  // Lock hold az once: prefer value learned while in-pass; else az at crossing.
  // Never take a "losAz" that only appeared after the pass ended (next pass).
  if (!postPassHeld) {
    if (heldLosAz == null || !Number.isFinite(heldLosAz)) {
      if (look.az != null && Number.isFinite(look.az)) {
    ...
    console.log(
      ...
      "(az at min EL; no slew to next AOS until T-5 min)",
    );
  }

  // Only within 5 minutes of the *next* AOS: preposition to that min-EL AOS az.
  ...
  // Between passes (or next AOS further than 5 min): stay on held LOS az, EL 0.
  prepositioned = false;
  if (heldLosAz != null && Number.isFinite(heldLosAz)) {
    desiredAz = heldLosAz;
  }

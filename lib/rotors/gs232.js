...
function updateTracking(look, aosAz, losAz) {
  ...
  if (look.el >= minEl) {
    ...
    // Below horizon: HOLD at LOS az, el=0.
    // Do NOT slew to AOS — nextAosAz is often still *this* pass's AOS
    // (state passes[0] until the next pass becomes current), which causes
    // a long reverse swing right as the sat sets (W140 → W316 in the log).
    // Use Park for an explicit home position; next pass will re-acquire.
    modeLocked = false;
    flipped = false;
    const holdAz =
      nextLosAz != null && Number.isFinite(nextLosAz)
        ? nextLosAz
        : look.az != null && Number.isFinite(look.az)
          ? look.az
          : desiredAz;
    if (holdAz != null && Number.isFinite(holdAz)) {
      const azN = normalizeAz(holdAz);
      if (
        desiredAz == null ||
        Math.abs(shortestDelta(desiredAz, azN)) >= 1 ||
        (desiredEl != null && desiredEl > 0.5)
      ) {
        console.log(
          "GS-232 below horizon — hold LOS az",
          azN.toFixed(1),
          "el 0 (not prepositioning to AOS)",
        );
      }
      desiredAz = azN;
    }

/**
 * Yaesu FT-991 / FT-991A ASCII CAT driver.
 *
 ...
 * Dual serial: Radio 1 = DL, Radio 2 = UL.
 * "Other (Yaesu CAT)" also routes here (best-effort ASCII).
 */
...
function setCtcss(which) {
  ...
  lastCtcssApplied = null;
  console.log("FT991 CTCSS", ctcssMode, activeCtcssHz());
...
async function applyCtcssToRadio() {
  ...
  try {
    if (hz != null) {
      ...
      console.log("FT991 CTCSS", hz, "Hz ON on", link.name);
    } else {
      await sendCmd(link, "CT0", 60);
      console.log("FT991 CTCSS OFF on", link.name);
    }
    lastCtcssApplied = key;
  } catch (e) {
    console.warn("FT991 CTCSS:", e.message);
  }
}
...
async function ensureSplit() {
  if (dualMode()) {
    if (splitOn && dl.connected) {
      ...
      splitOn = false;
      console.log("FT991 SPLIT OFF (dual — Radio 1 DL only)");
    }
    return false;
  }
  if (!txSplitEnabled()) {
  ...
  splitOn = true;
  console.log("FT991 SPLIT ON (TX = VFO B)");
  return true;
}
...
module.exports = {
  meta: {
    id: "ft991",
    label: "Yaesu FT-991/991A (ASCII CAT)",
    match(cfg) {
      if (typeof cfg.useFt991Serial === "function") return cfg.useFt991Serial();

...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // Frequency first (drives band stack), then mode — MAIN = UL
    if (ulTarget != null) {
      const f = await setLinkFrequency(dl, ulTarget, force, "A");
    ...
    // SUB = DL
    if (dlTarget != null) {
      const f = await setLinkFrequency(dl, dlTarget, force, "B");

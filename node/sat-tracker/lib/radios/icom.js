 ...
 *
 *  SPLIT (Radio 2 blank):
 *    One serial port. VFO A = DL, VFO B = UL, SPLIT ON.
 *
...
function deviceFor(link) {
  if (link.name === "ul" && dualMode()) {
    return {
      ...
      civ: config.CAT2_CIV_ADDR != null ? config.CAT2_CIV_ADDR : config.CAT_CIV_ADDR,
    };
  }
  return {
...
function markSyncNeeded(reason) {
  ...
  dl.lastHz = null;
  ul.lastHz = null;
  console.log("Icom syncNeeded:", reason || "enable", dualMode() ? "(dual)" : "(split)");
}
...
async function sendCiv(link, command) {
  if (!link.port || !link.connected) throw new Error("Icom " + link.name + " not connected");
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      p.open((err) => {
        ...
        p.on("error", (e) => console.warn("Icom", link.name, "error:", e.message));
        console.log(
          "Icom",
...
async function applyCtcssToRadio() {
  ...
  try {
    if (hz != null) {
      await sendCiv(link, Buffer.concat([Buffer.from([0x1b, 0x00]), toneToBcd(hz)]));
      await sendCiv(link, Buffer.from([0x16, 0x42, 0x01]));
      console.log("Icom CTCSS", hz, "Hz ON on", link.name);
    } else {
      await sendCiv(link, Buffer.from([0x16, 0x42, 0x00]));
      console.log("Icom CTCSS OFF on", link.name);
    }
...
async function selectVfoA(link) {
  try {
    await sendCiv(link, Buffer.from([0x07, 0x00]));
    return true;
  } catch (e) {
    return false;
  }
  ...
  if (dualMode()) {
    ...
    if (splitOn && dl.connected) {
      try {
        ...
        splitOn = false;
        console.log("Icom SPLIT OFF (dual mode — Radio 1 is DL only)");
      } catch (_) {}
    }
    return false;
  }
 ...
 * In split mode on DL link: which "A"|"B" selects VFO.
 * In dual mode: always VFO A on that radio.
 */
async function setLinkFrequency(link, freqHz, force, vfo) {
  ...
  if (!force && !syncNeeded && link.lastHz != null && Math.abs(target - link.lastHz) < 1) {
    return true;
  }

  // Dual: always VFO A. Split on single radio: respect vfo.
  const useVfoB = !dualMode() && vfo === "B";
  const sub = useVfoB ? 0x01 : 0x00;
  const bcd = freqToBcd(target);

  for (let attempt = 1; attempt <= FREQ_RETRIES; attempt++) {
    try {
      const reply = await sendCiv(link, Buffer.concat([Buffer.from([0x25, sub]), bcd]));
      if (reply.includes(0xfa)) {
        await sendCiv(link, Buffer.from([0x07, sub]));
        const r2 = await sendCiv(link, Buffer.concat([Buffer.from([0x05]), bcd]));
        if (r2.includes(0xfa)) {
          console.warn("Icom", link.name, "VFO", useVfoB ? "B" : "A", "freq NAK attempt", attempt);
          await sleep(80 * attempt);
          continue;
        }
        if (useVfoB) await selectVfoA(link);
      }
      link.lastHz = target;
...
async function setLinkMode(link, modeCode, modeName, force, vfo) {
  if (!link.connected) return false;
  if (!force && !syncNeeded && link.lastMode === modeCode) return true;
  ...
  for (let attempt = 1; attempt <= MODE_RETRIES; attempt++) {
    try {
      const reply = await sendCiv(link, Buffer.from([0x26, sub, modeCode, 0x01]));
      if (reply.includes(0xfa)) {
        await sendCiv(link, Buffer.from([0x07, sub]));
        await sendCiv(link, Buffer.from([0x06, modeCode, 0x01]));
        if (useVfoB) await selectVfoA(link);
      }
...
async function pushFrequencies(ulHz, dlHz) {
  ...
  // Dual: open UL link on Radio 2. Split: UL is VFO B on same port.
  if (dual && hasUl) {
    ul.wanted = true;
    if (!ul.connected) {
      await openLink(ul);
  ...
  if (dual) {
    ...
    if (hasDl && dl.connected) {
      ...
      if (!m || !f) ok = false;
        ...
        console.log("Icom DL (R1)", (Math.round(dlHz) / 1e6).toFixed(6), "MHz [dual]");
      }
    }
    if (hasUl && ul.connected) {
      ...
      if (!m || !f) ok = false;
        ...
        console.log("Icom UL (R2)", (Math.round(ulHz) / 1e6).toFixed(6), "MHz [dual]");
      }
    }
  } else {
    // ── SPLIT: single radio, VFO A = DL, VFO B = UL, SPLIT ON ──
    if (!dl.connected) {
      maybeClearSync(false);
      return;
    }
    ...
    if (hasDl) {
      ...
      if (!m || !f) ok = false;
        ...
        console.log("Icom VFO A (DL)", (Math.round(dlHz) / 1e6).toFixed(6), "MHz [split]");
      }
    }
    if (hasUl) {
      ...
      if (!m || !f) ok = false;
        ...
        console.log("Icom VFO B (UL)", (Math.round(ulHz) / 1e6).toFixed(6), "MHz [split]");
      }
    }
    await selectVfoA(dl);
  }

...
function readFormConfig() {
  ...
  // SmartSDR only speaks CAT (not TCI); allow rigctl for any type
  if (radioType === "smartsdr" && radioProtocol === "tci") radioProtocol = "cat";
...
function updateProtocolOptions() {
  ...
  add("rigctl", "rigctl (Hamlib)");

  if (radioType === "smartsdr" && current === "tci") {
    proto.value = "cat";
  } else if (current === "tci" || current === "cat" || current === "rigctl") {
    proto.value = current;
  } else {
    proto.value = "cat";
  }
...
function updateRadioFormVisibility() {
  ...
  if (transport === "tcp") {
    ...
    const isRigctl = protocol === "rigctl";
    const isCatTcp =
      protocol === "cat" &&
      (radioType === "smartsdr" ||
        radioType === "aethersdr" ||

...
async function programSplitVfOs(dlHz, ulHz, mods, force) {
  ...
  const SETTLE_MS = 2000; // FT-817 needs long gaps between VFO ops

  async function setOnCurrent(hz, modeCode, label) {
    // Always send mode — per-VFO on the 817
    dl.lastMode = null;
    const okMode = await sendCmd(dl, [modeCode, 0, 0, 0], CMD.SET_MODE, 0, 150);
    await sleep(SETTLE_MS);
    const bcd = freqToBcd(Math.round(hz));
    const okFreq = await sendCmd(dl, bcd, CMD.SET_FREQ, 0, 150);
    await sleep(SETTLE_MS);
    if (okMode == null || okFreq == null) return false;
    dl.lastMode = modeCode;
    dl.lastHz = Math.round(hz);
    console.log(
      "FT817",
      label,
      (Math.round(hz) / 1e6).toFixed(6),
      "MHz mode",
      modeCode,
    );
    return true;
  ...
  console.log("FT817 split program start (slow, 2s settles)");
  ...
  // --- Ensure we are on VFO A for DL ---
  // FT-817 has only toggle. Convention: operator starts on VFO A, or we
  // toggle once if the displayed freq is already near UL (stuck on B).
  ...
  // Software mark: we treat current as A for the rest of this sequence
  dl.vfo = "A";
  ...
  // --- SPLIT ON (still on B), then back to A ---
  await setSplit(true);
  console.log("FT817 SPLIT ON");
  await sleep(SETTLE_MS);

...
async function sessionOpen(device, baud) {
  ...
  /** Poll AI1 until we get a reading near target, or timeout. */
  state.readPosAfterMove = async (target, opts) => {
    ...
    const minWait = (opts && opts.minWaitMs) || 1500;
    await sleep(minWait);
    ...
    while (Date.now() < deadline) {
      ...
      const err =
        target != null && Number.isFinite(target)
          ...
          : 0;
      // For EL use linear distance
      const errEl =
        target != null && Number.isFinite(target)
          ? Math.abs(last.pos - target)
          : 0;
      const useEl = opts && opts.isEl;
      const close = useEl ? errEl <= settleDeg : err <= settleDeg;
      if (prev != null && Math.abs(last.pos - prev) < 0.5) stable += 1;
      else stable = 0;
      prev = last.pos;
      if (close && stable >= 1) return last;
      // Also accept if we've been stable even if still moving slowly

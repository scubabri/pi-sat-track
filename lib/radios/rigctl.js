...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    // After initial sync: rate-limit; skip tiny Doppler steps only
    ...
    if (needMode || firstDual) await setMode(dl, mods.dl, "main");
    ...
    if (needMode || firstDual) await setMode(dl, mods.ul, "sub");

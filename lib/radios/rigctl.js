...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  // LOCK on:  write Main (DL) + Sub (UL) each interval
  // LOCK off: write Sub (UL) only — Main is the operator dial (no Hamlib read in SAT)
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    if (locked) {
      // Full Doppler: always push both sides when either moved (FF, no await)
      if (!dlMoved && !ulMoved) return;
      writeCmd(dl, "V Main");
      writeCmd(dl, "F " + dlT);
      writeCmd(dl, "V Sub");
      writeCmd(dl, "F " + ulT);
      ...
      // Unlocked: Sub/UL tracks Doppler; Main left alone for dial
      // Keep lastFreqHz aligned to target so we don't accumulate "Main debt"
      dl.lastFreqHz = dlT;
      if (!ulMoved) return;
      writeCmd(dl, "V Sub");
      writeCmd(dl, "F " + ulT);
      ...
      dl.setCount = (dl.setCount || 0) + 1;
      if (dl.setCount <= 5 || dl.setCount % 8 === 0) {
        console.log("rigctl Doppler Sub/UL", (ulT / 1e6).toFixed(6), "MHz");
      }
    }
    return;
  }

...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    if (ulTarget != null && dlTarget != null) {
      ...
      // Probe: cross-band 0x05 NAKs when VFO is on the wrong band stack.
      // If MAIN holds DL band (or SUB holds UL band), exchange first so
      // each 0x05 is same-band and ACKs reliably.
      if (wantMain !== "other" && wantSub !== "other" && wantMain !== wantSub) {
        if (
          (haveMain && haveMain === wantSub) ||
          (haveSub && haveSub === wantMain) ||
          (haveMain && haveMain !== wantMain)
        ) {
          console.log(
            "IC9700 MAIN↔SUB exchange (MAIN band",
            haveMain || "?",
            "→ need",
            wantMain + ")",
          );

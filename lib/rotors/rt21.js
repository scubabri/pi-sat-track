 ...
 * (az+180, 180-el) to minimize AZ flips through a pass.
 */
 ...
 * Sticky across ticks to minimize AZ flips; only switches when the other
 * representation is clearly closer to the current rotor pose.
 */
let flipped = false;
  ...
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      const timer = setTimeout(() => {
        // Abort this open attempt only — do not leave listeners that schedule reconnect.
        try {
      ...
      port.open((err) => {
        if (settled) {
          // Timed out already — if open still succeeded, close quietly
          if (!err && port.isOpen) {
            try {
              intentionalClose = true;
              port.close(() => {
        ...
        axis.stillCount = 0;
        axis.stallRetries = 0;
        // Fresh position after open — do not trust pre-disconnect values
        axis.pos = null;
        axis.lastPos = null;
        axis.lastCmd = null;
...
async function commandPos(axis, degrees) {
  ...
  if (nnn < 0) nnn = 0;
  // AZ: 0-360, EL: 0-180 (180° elevation mounts)
  if (axis.name === "el") {
...
function disconnect() {
  ...
  // Invalidate any in-flight park-complete watcher
  parkSession += 1;
  stopPoll();
  antennaOn = false;
  parkHold = false;
  ...
  // close callbacks may run async; clear flag shortly after
  setTimeout(() => {
    intentionalClose = false;
  }, 150);
  desiredAz = null;
 ...
 * Picking flipped is the "over the top" path — same beam, no AZ snap
 * when the geometry would otherwise demand a 180° azimuth jump.
 */
...
function motionCost(fromAz, fromEl, toAz, toEl) {
  ...
  // Weight AZ higher so we avoid 180° flips unless EL savings are large
  return dAz * 1.5 + dEl;
}

/**
 * Choose normal vs flipped pointing to minimize motion from current pose.
 * Sticky hysteresis (~20°) keeps the same side of the mount through a pass.
 */
function choosePointing(satAz, satEl) {
  const cands = pointingCandidates(satAz, satEl);
  const curAz = az.pos;
  const curEl = el.pos;
  let best = cands[flipped ? 1 : 0];
  let bestCost = Infinity;
  for (const c of cands) {
    let cost = motionCost(curAz, curEl, c.az, c.el);
    // Prefer staying in the current mode unless the other is clearly better
    if (c.flipped === flipped) cost -= 20;
    if (cost < bestCost) {
      bestCost = cost;
      best = c;
    }
...
function updateTracking(look, aosAz) {
  ...
  if (look.el >= config.ROTOR_MIN_EL) {
    const p = choosePointing(look.az, look.el);
    desiredAz = p.az;
    desiredEl = p.el;
  } else {
    // Below min el: preposition AZ for next AOS at horizon (EL 0, not flipped)
    flipped = false;
    desiredAz =
...
function park() {
  ...
  // New session — kills any previous park-complete watcher
  parkSession += 1;
  ...
  // Force a fresh CMD even if lastCmd already matched park
  az.lastCmd = null;
  el.lastCmd = null;
  az.state = "IDLE";
  el.state = "IDLE";
...
function watchParkComplete(session, paz, pel) {
  ...
  const tick = () => {
    parkCompleteTimer = null;
    // Stale session or user cancelled
    if (session !== parkSession) return;
    if (!antennaOn || !parkHold) return;

    // Require live connections and a real position read this session
    // (pos is cleared on open/disconnect so stale values cannot complete early)
  ...
  // Give ports time to open and first CMD to go out before first check

...
function parseC2(text) {
  ...
  // Preferred: AZ=090EL=030 or AZ=90 EL=30 (with optional +)
  let m = s.match(
  ...
  // Fox Delta / some clones: "+090+030" or "090 030" or bare digit groups
  // Prefer 3-digit groups first (canonical GS-232), then any number pairs
  let nums = s.match(/([+-]?\d{3})(?:\.\d+)?/g);
  ...
  if (nums && nums.length >= 2) {
    ...
    if (Number.isFinite(a) && Number.isFinite(e) && a >= -5 && a <= 450 && e >= -5 && e <= 180) {
      return { az: a, el: e };
    }
  }
  return null;
...
async function queryPosition() {
  ...
  if (pos) {
    ...
    // Log once-ish so we can diagnose Fox Delta / clone reply formats
    console.warn("GS-232 C2 parse fail raw:", JSON.stringify(String(raw).slice(0, 80)));
  }
  return pos;
}
...
let lastGotoAt = 0;
const GOTO_REISSUE_MS = 4000; // don't spam W while moving / if C2 is flaky
...
async function pollLoop() {
  ...
  if (desiredAz != null && Number.isFinite(desiredAz)) {
    ...
    const needAz =
      az == null || absErr(desiredAz, az) >= dead;
    const needEl =
      !azOnly() &&
      desiredEl != null &&
      Number.isFinite(desiredEl) &&
      (el == null || Math.abs(desiredEl - el) >= dead);

    // If we already commanded this target recently, wait before re-issuing
    // (stops continuous relay chatter when position feedback is missing/flaky)
    const sameTarget =
      lastCmdAz != null &&
      Math.abs(lastCmdAz - Math.round(desiredAz)) < 1 &&
      (azOnly() ||
        (lastCmdEl != null &&
          desiredEl != null &&
          Math.abs(lastCmdEl - Math.round(desiredEl)) < 1));
    const recentlyCommanded =
      sameTarget && Date.now() - lastGotoAt < GOTO_REISSUE_MS;

    if ((needAz || needEl) && !recentlyCommanded) {
      const targetEl = azOnly()
        ? 0
        : desiredEl != null && Number.isFinite(desiredEl)
          ? desiredEl
          : el || 0;
      const ok = await commandGoto(desiredAz, targetEl);
      if (ok) lastGotoAt = Date.now();
    } else if (!needAz && !needEl) {
      azState = "IDLE";
...
function updateTracking(look, aosAz) {
  ...
  if (look.el >= (config.ROTOR_MIN_EL || 0)) {
    ...
    // Below min el: preposition AZ for next AOS (Park button is separate)
    desiredAz =
...
function park() {
  ...
  const pel =
    config.ROTOR_PARK_EL != null && Number.isFinite(config.ROTOR_PARK_EL)
      ...
      : 0;
  console.log("GS-232 park → az", paz, azOnly() ? "(AZ only)" : "el " + pel);
  parkHold = true;
  desiredAz = paz;
  desiredEl = azOnly() ? 0 : pel;
  ...
  const tick = () => {
    ...
    if ((azOk && elOk) || noFeedbackOk || timedOut) {
      console.log(
        ...
        timedOut ? "(timeout)" : noFeedbackOk ? "(no C2 feedback, assumed ok)" : "",
        "az",
        az,
        "el",
        el,

  ...
  if (dualMode()) {
    ...
    if (hasUl && !hasDl) {
      ...
      if (force || syncNeeded || !splitOn) {
        ...
        // Tracking: update RX (DL) only most ticks — avoids constant A/B flipping.
        // UL refreshed every ~2s via lastUlTrack timestamp.
        const now = Date.now();
        if (!dl._lastUlTrack) dl._lastUlTrack = 0;
        const refreshUl = now - dl._lastUlTrack > 2000;

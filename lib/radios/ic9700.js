 ...
 *    Dualwatch ON (hear SUB while MAIN selected)
 *    Cross-band: select VFO then 0x05 so the 9700 switches band stack;
 *    if MAIN/SUB are inverted vs target bands, try MAIN↔SUB exchange (0x07 B0).
 *
...
async function ensureSplitOff() {
  ...
  if (!splitOn) {
    // Still send OFF once after open/sync so radio matches software
    if (!syncNeeded) return true;
  }
  try {
...
/** Dualwatch so SUB (DL) is audible while MAIN (UL) is selected for TX. */
async function ensureDualWatch(on) {
  if (dualMode() || !dl.connected) return false;
  try {
    const reply = await sendCiv(
      dl,
      Buffer.from([0x16, 0x5a, on ? 0x01 : 0x00]),
    );
    ...
    if (syncNeeded || on) {
      console.log("IC9700 Dualwatch", on ? "ON" : "OFF");
    }
    return true;
  } catch (e) {
...
async function setLinkFrequency(link, freqHz, force, vfo) {
  ...
  const thresh = force || syncNeeded ? FREQ_SYNC_THRESH_HZ : FREQ_DEADBAND_HZ;
  if (
    !force &&
    store.lastHz != null &&
  ...
  for (let attempt = 1; attempt <= FREQ_RETRIES; attempt++) {
    try {
      ...
      if (reply.includes(0xfa)) {
        ...
        if (reply.includes(0xfa)) {
          console.warn("Icom", link.name, band, "freq NAK attempt", attempt);
...
async function setLinkMode(link, mdCode, modeName, force, vfo) {
  ...
  // Mode only on change or during sync / force
...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // If MAIN/SUB band groups are inverted vs targets, exchange first
    if (hasUl && hasDl) {
      const wantMain = bandGroup(ulHz); // UL on MAIN
      const wantSub = bandGroup(dlHz); // DL on SUB
    ...
    // MAIN first = TX/UL (top VFO) — forces band switch on MAIN if needed
    if (hasUl) {
      const m = await setLinkMode(dl, mods.ul, mods.ulName, force, "A");
      const f = await setLinkFrequency(dl, Math.round(ulHz), force, "A");
      if (!m || !f) ok = false;
      else if (force || syncNeeded) {
        console.log(
          "IC9700 MAIN (UL/TX)",
    ...
    // SUB = DL/RX (bottom)
    if (hasDl) {
      const m = await setLinkMode(dl, mods.dl, mods.dlName, force, "B");
      const f = await setLinkFrequency(dl, Math.round(dlHz), force, "B");
      if (!m || !f) ok = false;
      else if (force || syncNeeded) {
        console.log(
          "IC9700 SUB (DL/RX)",
    ...
    // Leave MAIN selected so PTT / focus is on TX VFO
    await selectMain(dl);
  }

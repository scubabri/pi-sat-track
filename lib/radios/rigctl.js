...
/**
 * Force Satellite mode OFF on the rig (Hamlib set_func).
 * IC-9700 rejects get_freq (CI-V 0x25) while SAT is on → RPRT -9.
 * Harmless on radios that ignore SATMODE.
 */
async function ensureSatModeOff(link) {
  if (!link || !link.connected) return false;
  try {
    // Capital U = set_func; lowercase u is get_func only
    const resp = await sendCmd(link, "U SATMODE 0");
    if (resp == null) {
      console.warn("rigctl", link.name, "SATMODE OFF: no reply");
      return false;
    }
    if (/^RPRT\s+-/.test(resp)) {
      console.warn("rigctl", link.name, "SATMODE OFF:", resp);
      return false;
    }
    console.log("rigctl", link.name, "SATMODE OFF", resp.trim ? resp.trim() : resp);
    return true;
  } catch (e) {
    console.warn("rigctl", link.name, "SATMODE OFF error:", e.message || e);
    return false;
  }
}
...
function openLink(link, side) {
  ...
  return new Promise((resolve) => {
    ...
    sock.once("connect", () => {
      ...
      // IC-9700: get_freq (0x25) fails in SAT mode — clear it before use
      ensureSatModeOff(link)
        .catch(() => {})
        .finally(() => done(true));
    });
...
async function setFreq(link, hz) {
  ...
  if (isInitial) {
    ...
    if (resp != null && /^RPRT\s+-9\b/.test(resp)) {
      console.warn("rigctl", link.name, "set_freq rejected — forcing SATMODE OFF");
...
async function readFreq(link) {
  ...
  // IC-9700 in SAT mode: RPRT -9 — force SAT off and retry once
  if (/^RPRT\s+-9\b/.test(resp) || /rejected/i.test(resp)) {
    console.warn("rigctl", link.name, "get_freq rejected — forcing SATMODE OFF");
    await ensureSatModeOff(link);
    link.buf = "";
...
async function pushFrequencies(ulHz, dlHz) {
  ...
  if (dlHz != null && Number.isFinite(dlHz) && dl.connected) {
    ...
    // Skip absorb right after a reset/reconnect (lastFreqHz was null → initial set)
    // so a leftover dial position from the previous sat cannot stick a -kHz offset.

...
function computeTick() {
  ...
  let rotorAosAz = lastAosAz;
  let rotorLosAz = lastLosAz;
  ...
  try {
    ...
    for (let i = 0; i < passesR.length; i++) {
      ...
      if (tnow >= aosMs && tnow <= losMs) {
        rotorAosAz = p.aosAz != null ? p.aosAz : rotorAosAz;
        rotorLosAz = p.losAz != null ? p.losAz : rotorLosAz;
        if (passesR[i + 1]) {
      ...
      if (aosMs > tnow) {
        // Below / between passes — next rise
        rotorNextAosMs = aosMs;
        rotorNextAosAz = p.aosAz;
        // Keep last known losAz for post-pass hold (from lastAos/Los updates)
        break;
      }
    }
    if (rotorAosAz != null) lastAosAz = rotorAosAz;
    if (rotorLosAz != null) lastLosAz = rotorLosAz;
  } catch (e) {
...
function computeState() {
  ...
  if (passes.length && passes[0].aosAz != null) {
    ...
    lastLosAz = passes[0].losAz != null ? passes[0].losAz : null;
  }

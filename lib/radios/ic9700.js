 ...
 *    MAIN (top)    = UL / TX   — transmit on MAIN
 *    SUB  (bottom) = DL / RX   — listen / tune passband here
 *    SPLIT OFF (0x0F 0x00)     — required; SPLIT would key MAIN as TX on
 *                                the wrong side for a MAIN=DL layout
 ...
 *  dual → Radio 2 (UL)
 *  single → MAIN (UL/TX); select MAIN first so tone sticks to TX band
 */
async function applyCtcssToRadio() {
  ...
  try {
    ...
    if (hz != null) {
      ...
      console.log("IC9700 CTCSS", hz, "Hz ON on", dualMode() ? "ul" : "MAIN/TX");
    } else {
      await sendCiv(link, Buffer.from([0x16, 0x42, 0x00]));
      console.log("IC9700 CTCSS OFF on", dualMode() ? "ul" : "MAIN/TX");
    }
...
/**
 * Always keep SPLIT OFF.
 * On IC-9700 dualwatch, SPLIT causes TX on MAIN — which is fatal if MAIN
 * were the downlink. Our layout is MAIN=UL/TX, SUB=DL/RX, so SPLIT stays OFF.
 */
async function ensureSplitOff(force) {
  ...
  try {
    ...
    if (force || splitOn || syncNeeded) {
      console.log("IC9700 SPLIT OFF (TX = MAIN / top = UL)");
    }
    splitOn = false;
    return true;
  } catch (e) {
 ...
 * We operate non-SAT dualwatch: MAIN=UL/TX, SUB=DL/RX, SPLIT OFF.
 */
/**
 ...
 * Single-radio: MAIN → ul store (TX), SUB → dl store (RX).
...
function bandState(link, vfo) {
  ...
  // Single-radio: MAIN = UL/TX, SUB = DL/RX
...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // SPLIT OFF — TX is always MAIN. Do not use SPLIT on this radio for sats.
    if (!dl.connected) {
      maybeClearSync(false);
      return;
    }
    ...
    // SAT OFF + Dualwatch + SPLIT OFF (TX=MAIN). Skip once applied on Doppler ticks.
    ...
    // band stack. MAIN must be on the UL band before we set.
    if (
      ulTarget != null &&
      dlTarget != null &&
...
/**
 * When LOCK is off: read SUB (DL) VFO and fold operator tuning into
...
async function pollVfo() {
  ...
  try {
    // SUB = DL/RX on single-radio layout (bottom)
    const measured = await getSelectedFrequency(dl, "B");
    ...
    // Restore TX focus on MAIN after reading SUB
    await selectMain(dl);
    broadcastStatus();
  } catch (_) {
 ...
 * Also re-asserts SAT OFF + Dualwatch + SPLIT OFF (TX=MAIN).
 */
async function runConnectionTest() {
  ...
  try {
    ...
    let msg = "CI-V MAIN(UL/TX) " + (m || "?") + " MHz";
    if (s) msg += " / SUB(DL) " + s + " MHz";
    msg += " | SAT OFF, Dualwatch ON, SPLIT OFF (TX=MAIN)";
    return {
      ok: true,
      message: msg,
      detail: {

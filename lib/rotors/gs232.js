 ...
 *   C2          → AZ=nnnEL=nnn  (position poll; may be silent/flaky on Fox Delta)
 ...
 *
 * 180° EL / N-stop flip (when ROTOR_EL_MAX=180): same geometry as RT-21 —
 * FLIPPED commands (az+180, 180-el) so AZ never walks across north stop.
 *
 * Config:
...
function parseC2(text) {
  ...
  // Preferred: AZ=090EL=030 or AZ=90 EL=30 (optional + / spaces / =)
  let m = s.match(
  ...
  // Fox Delta / clones: "+090+030", "090 030", "090,030"
...
const GOTO_NO_FEEDBACK_IDLE_MS = 10000;
  ...
  if (desiredAz != null && Number.isFinite(desiredAz)) {
    ...
    if (hasFeedback) {
      ...
      if (!needAz && !needEl) {
        ...
        // Off target with real feedback — correct with another W
        await commandGoto(desiredAz, targetEl);
      }
    } else {
      // No usable C2: command once per target; do not hammer W
      if (!same || now - lastGotoAt >= GOTO_NO_FEEDBACK_REISSUE_MS) {
        await commandGoto(desiredAz, targetEl);
      } else if (same && now - lastGotoAt >= GOTO_NO_FEEDBACK_IDLE_MS) {
        // Assume move is underway / done; stop looking "stuck"
        azState = "IDLE";
        elState = azOnly() ? "N/A" : "IDLE";
      }
    }
...
function updateTracking(look, aosAz, losAz) {
  ...
  const minEl = config.ROTOR_MIN_EL != null ? config.ROTOR_MIN_EL : 0;
  ...
  if (look.el >= minEl) {
    ...
    // Below horizon — preposition for next AOS. Crossing N → start flipped.
    modeLocked = false;
    const aos =
      nextAosAz != null && Number.isFinite(nextAosAz) ? nextAosAz : look.az;
    const willFlip = flipEnabled() && passCrossesNorthStop(aos, nextLosAz);
    flipped = willFlip;
    if (willFlip) {
      desiredAz = normalizeAz(Number(aos) + 180);
      desiredEl = elMax();
    } else {
      desiredAz = normalizeAz(aos);
      desiredEl = 0;
    }
  }
}

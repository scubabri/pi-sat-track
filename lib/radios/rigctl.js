...
async function pushFrequencies(ulHz, dlHz) {
  ...
  if (hasUl && hasDl && dl.connected && (await canUseDualSat(dl))) {
    ...
    const needFreq =
      dlDelta >= DUAL_FREQ_THRESH_HZ || ulDelta >= DUAL_FREQ_THRESH_HZ;
    const needMode =
      dl.lastMode !== mods.dl || dl.lastSubMode !== mods.ul;
    const due = now - (dl.lastPushAt || 0) >= DUAL_PUSH_MIN_MS;

    // After initial sync: rate-limit; skip tiny Doppler steps
    if (!firstDual && dl.lastFreqHz != null) {
      if (!needFreq && !needMode) return;
      if (!due && dlDelta < 200 && ulDelta < 200) return;
      // Unlocked: do not fight the dial (get_freq unreliable in SAT mode)
      if (!locked && !needMode && dlDelta < 5000 && ulDelta < 5000) {
        return;
      }
    }
    ...
    // MAIN = downlink (RX / dial)
    await setVfo(dl, "Main");
    await setMode(dl, mods.dl, "main");
    if (needFreq || firstDual || dl.lastFreqHz == null) {
    ...
    if (needFreq || firstDual || dl.lastSubHz == null) {

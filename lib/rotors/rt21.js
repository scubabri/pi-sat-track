 ...
 *   If AOS→LOS crosses north, START flipped (east side, EL≈180 at horizon)
 *   so AZ walks south-side and never hits the hard north stop.
 */
...
/** North-stop at 0°: true if the shortest path from→to crosses north. */
...
/**
 * True when the pass geometry forces an AZ path across the N stop (0°).
 * With a north-hard-stop RT-21 we start FLIPPED (east side, EL≈180) so
 * AZ walks the long way around south and never hits the stop.
 */
function passCrossesNorthStop(aosAz, losAz) {
  ...
  return shortestPathCrossesNorth(aosAz, losAz);
}

/**
 * Choose mode for the whole pass up front:
 *   - FLIPPED if AOS→LOS (or AOS→current) crosses north
 *   - else NORMAL, unless rotor is already over-top (EL>100)
 */
function chooseInitialMode(satAz, satEl) {
  const aos =
    nextAosAz != null && Number.isFinite(nextAosAz) ? nextAosAz : satAz;
  if (passCrossesNorthStop(aos, nextLosAz)) {
    flipped = true;
    return;
  }
  // Mid-pass acquire: AOS→current already crosses N
...
/**
 * N-stop over-the-top:
 *   If the pass crosses north, START flipped (east side / EL≈180 at AOS)
 *   and stay flipped for the whole pass so AZ never crosses the N stop.
 *   Preposition below horizon uses the same flipped AOS pointing.
 */
function updateTracking(look, aosAz, losAz) {
  ...
  if (look.el >= config.ROTOR_MIN_EL) {
    if (!modeLocked) {
      ...
      console.log(
        "Rotor mode",
        flipped ? "FLIPPED (N-stop over-top)" : "NORMAL",
        "sat az",
        look.az.toFixed(1),
        "el",
        look.el.toFixed(1),
        "aos",
        nextAosAz != null ? nextAosAz.toFixed(1) : "-",
        "los",
        nextLosAz != null ? nextLosAz.toFixed(1) : "-",
      );
    }
    applyMode(look.az, look.el);
  } else {
    // Below horizon — preposition for next AOS. If that pass crosses N,
    // start already flipped: AZ ≈ AOS+180 (~east), EL = 180 (over-top horizon).
    modeLocked = false;
    const aos =
      nextAosAz != null && Number.isFinite(nextAosAz) ? nextAosAz : look.az;
    const willFlip = passCrossesNorthStop(aos, nextLosAz);
    flipped = willFlip;
    if (willFlip) {
      desiredAz = normalizeAz(Number(aos) + 180);
      desiredEl = EL_MAX;
    } else {
      desiredAz = aos;
      desiredEl = 0;

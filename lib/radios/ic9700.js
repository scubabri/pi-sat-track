 ...
 *
 * Single-radio (Radio 2 blank):
 *
 *  FM sats → Satellite mode ON (0x16 0x5A 0x01):
 *    MAIN (top) = DL / RX   — main dial tunes downlink
 *    SUB (bottom) = UL / TX — TX always on SUB in SAT mode (no SPLIT)
 *    Leave MAIN selected so dial stays on DL
 *
 *  Linear / SSB → Satellite mode OFF, dualwatch:
 *    MAIN (top) = UL / TX
 *    SUB (bottom) = DL / RX
 *    SPLIT OFF, Dualwatch ON
 *
...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // ── SINGLE radio ──
    // FM  → SAT ON:  MAIN=DL (dial), SUB=UL (TX) — no SPLIT
    // SSB → SAT OFF: MAIN=UL (TX),  SUB=DL (RX), dualwatch, SPLIT OFF
    if (!dl.connected) {
      maybeClearSync(false);
      return;
    }

    const useSat = isFmMode(active && active.mode);
    let forceTune = force || syncNeeded;
    if (lastUseSat !== null && lastUseSat !== useSat) {
      forceTune = true;
      console.log(
        "IC9700 layout switch →",
        useSat ? "SAT (FM)" : "dualwatch (linear)",
      );
    }
    if (
      !forceTune &&
    ...
    // MAIN must already be on the band we will write first
    const mainWantBand = useSat ? bandGroup(dlTarget) : bandGroup(ulTarget);
    const mainStoreBand = useSat
      ? bandGroup(dl.lastHz)
      : bandGroup(ul.lastHz);
    if (
      ulTarget != null &&
      ...
      if (haveMain === "other" && forceTune) {
        ...
        if (mainHz != null) {
          ...
          if (useSat) dl.lastHz = mainHz;
          else ul.lastHz = mainHz;
        }
      }
      if (
        haveMain !== "other" &&
    ...
    if (useSat) {
      ...
      if (ulTarget != null && dlTarget != null && (force || syncNeeded)) {
        ...
        if (
          ...
          console.warn(
            "IC9700 MAIN still on UL band after SAT set — exchanging",
          );
          await exchangeMainSub();
    ...
    // Leave MAIN selected: dial = DL in SAT, dial = UL in linear
    if (forceTune) {
      await selectMain(dl);
...
/**
 * When LOCK is off: fold operator dial into offsets so Doppler does not
 * yank VFOs back to geometric center.
 *
 * SAT (FM): MAIN = DL → manualDlOffset += Δ
 * Linear:   MAIN = UL (dial is here). ulFineOffset += Δ (keep UL),
 *           manualDlOffset -= Δ (inverting DL tracks opposite on SUB)
 */
  ...
  try {
    if (satModeOn) {
      ...
      if (Math.abs(delta) > 50000) {
        console.warn("IC9700 VFO ignore large Δ", delta, "Hz [SAT DL]");
        return;
      }
      ...
      // Linear: dial is on MAIN = UL
      if (ul.lastHz == null || !Number.isFinite(ul.lastHz)) return;
      const measured = await getSelectedFrequency(dl, "A");
      if (measured == null || !isPlausibleRigHz(measured)) return;
      if (bandGroup(measured) !== bandGroup(ul.lastHz)) return;

      const delta = Math.round(measured - ul.lastHz);
      if (Math.abs(delta) < VFO_THRESH_HZ) return;
      if (Math.abs(delta) > 50000) {
        console.warn("IC9700 VFO ignore large Δ", delta, "Hz [linear UL]");
        return;
      }
      // Hold UL at dialled frequency on subsequent Doppler pushes
      ulFineOffset = clampOffset(ulFineOffset + delta);
      // Inverting: UL up → DL down
      manualDlOffset = clampOffset(manualDlOffset - delta);
      ul.lastHz = measured;
      dl.lastHz = null; // force SUB retune
      lastPushAt = 0;
      console.log(
        "IC9700 VFO UL",
        delta > 0 ? "+" + delta : String(delta),

 ...
 *    MAIN (top)    = DL / RX   — listen / tune passband here
 *    SUB  (bottom) = UL / TX   — transmit VFO
 *    SPLIT ON (0x0F 0x01)      — TX is on SUB
 ...
 *  dual → Radio 2 (UL)
 *  single → SUB (UL/TX); select SUB first so tone sticks to TX band
 */
async function applyCtcssToRadio() {
  ...
  try {
    if (!dualMode()) await selectSub(link);
    if (hz != null) {
      await sendCiv(
        link,
      ...
      console.log("IC9700 CTCSS", hz, "Hz ON on", dualMode() ? "ul" : "SUB/TX");
    } else {
      await sendCiv(link, Buffer.from([0x16, 0x42, 0x00]));
      console.log("IC9700 CTCSS OFF on", dualMode() ? "ul" : "SUB/TX");
    }
...
/**
 * Dual-radio: keep SPLIT OFF on Radio 1.
 * Single-radio uses ensureSplitOn (TX on SUB).
 */
...
/**
 * Single-radio layout: SPLIT ON → TX on SUB (bottom = UL).
 * MAIN (top) stays the RX / tune VFO (DL).
 */
async function ensureSplitOn(force) {
  if (!dl.connected) return false;
  if (!force && splitOn && !syncNeeded && satModeOffApplied) return true;
  try {
    const reply = await sendCiv(dl, Buffer.from([0x0f, 0x01]));
    if (reply.includes(0xfa)) {
      console.warn("IC9700 SPLIT ON NAK");
      return false;
    }
    if (force || !splitOn || syncNeeded) {
      console.log("IC9700 SPLIT ON (TX = SUB / bottom = UL)");
    }
    splitOn = true;
    return true;
  } catch (e) {
    console.warn("IC9700 SPLIT ON:", e.message);
    return false;
  }
}
 ...
 * In SAT mode many freq/mode ops return FA (NAK).
 * We operate non-SAT dualwatch: MAIN=DL/RX, SUB=UL/TX, SPLIT ON.
 */
/**
 ...
 * Single-radio: MAIN → dl store (RX), SUB → ul store (TX).
...
async function setLinkFrequency(link, freqHz, force, vfo) {
  ...
  for (let attempt = 1; attempt <= FREQ_RETRIES; attempt++) {
    try {
      ...
      // Leave TX focus on SUB after UL ops; MAIN after DL ops
      if (useSub) await selectSub(link);
...
async function pushFrequenciesBody(ulHz, dlHz) {
  ...
  if (dual) {
    ...
    // ── SINGLE: MAIN (top) = DL/RX, SUB (bottom) = UL/TX ──
    // Non-SAT dualwatch + SPLIT ON so TX is on SUB.
    if (!dl.connected) {
      maybeClearSync(false);
      return;
    }
    ...
    // SAT OFF + Dualwatch + SPLIT ON (TX=SUB). Skip once applied on Doppler ticks.
    await ensureSatModeOff(forceTune);
    await ensureDualWatch(true);
    await ensureSplitOn(forceTune);
    ...
    // band stack. MAIN must be on the DL band before we set.
    if (
      ulTarget != null &&
      dlTarget != null &&
      ...
      let haveMain = bandGroup(dl.lastHz);
      ...
      if (haveMain !== "other" && haveMain !== bandGroup(dlTarget)) {
        console.log(
    ...
    // Verify MAIN is on DL band; if still inverted, exchange + re-apply once
    if (ulTarget != null && dlTarget != null && (force || syncNeeded)) {
      ...
      if (
        ...
        console.warn(
          "IC9700 MAIN still on UL band after set — exchanging MAIN↔SUB",
        );
        await exchangeMainSub();
    ...
    // Keep TX on SUB: SPLIT ON + SUB selected after layout pass
    if (forceTune) {
      await ensureSplitOn(true);
      await selectSub(dl);
    }
  }
...
/**
 * When LOCK is off: read MAIN (DL) VFO and fold operator tuning into
 * manualDlOffset so the app + UL (inverting) track the passband.
 *
 * Safety (avoids the old ~10 GHz poison):
 *  - only when unlocked, not syncing, not mid-push
 *  - frame-walked parse + band plausibility
 *  - ignore |Δ| < VFO_THRESH or > ±50 kHz
 *  - clamp cumulative manual offset to ±MAX_OFFSET_HZ
 */
async function pollVfo() {
  if (locked || !radioOn || dualMode()) return;
  if (!dl.connected || pushInFlight || syncNeeded) return;
  // Let Doppler finish settling before reading the VFO
  if (lastPushAt && Date.now() - lastPushAt < PUSH_MIN_MS) return;
  if (dl.lastHz == null || !Number.isFinite(dl.lastHz)) return;

  try {
    // MAIN = DL/RX on single-radio layout (top)
    const measured = await getSelectedFrequency(dl, "A");
    if (measured == null || !isPlausibleRigHz(measured)) return;
    if (bandGroup(measured) !== bandGroup(dl.lastHz)) return;

    const delta = Math.round(measured - dl.lastHz);
    if (Math.abs(delta) < VFO_THRESH_HZ) return;
    // Passband-sized moves only (RS-44 ~60 kHz); larger = bad parse / band hop
    if (Math.abs(delta) > 50000) {
      console.warn(
        "IC9700 VFO ignore large Δ",
        delta,
        "Hz (measured",
        (measured / 1e6).toFixed(6),
        "last",
        (dl.lastHz / 1e6).toFixed(6) + ")",
      );
      return;
    }

    const next = clampOffset(manualDlOffset + delta);
    if (next === manualDlOffset) return;

    manualDlOffset = next;
    // Stay where the operator tuned so the next push does not yank DL back
    dl.lastHz = measured;
    // Force UL retune (inverting: UL moves opposite to DL offset)
    ul.lastHz = null;
    lastPushAt = 0;
    console.log(
      "IC9700 VFO DL",
    ...
    // Restore TX focus on SUB after reading MAIN
    await selectSub(dl);
    broadcastStatus();
  } catch (_) {
    // busy / transient — ignore
  }
}
 ...
 * Also re-asserts SAT OFF + Dualwatch + SPLIT ON (TX=SUB).
 */
async function runConnectionTest() {
  ...
  try {
    ...
    let msg = "CI-V MAIN(DL) " + (m || "?") + " MHz";
    if (s) msg += " / SUB(UL/TX) " + s + " MHz";
    msg += " | SAT OFF, Dualwatch ON, SPLIT ON (TX=SUB)";
    return {
      ok: true,
      message: msg,
      detail: {
